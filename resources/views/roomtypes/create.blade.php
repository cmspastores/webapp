...
.btn-save:hover { background:linear-gradient(90deg,#F4C38C,#E6A574); }
...
.form-actions { display:flex; gap:16px; justify-content:flex-end; margin-top:20px; }
    ...
    <div class="container">
        <div class="card">
            <div class="rooms-header">Add Room Type</div>
            <form action="{{ route('roomtypes.store') }}" method="POST">
                ...
                <div>
                    ...
                    <select name="is_transient" id="is_transient">
                        <option value="0" {{ old('is_transient', 0) == 0 ? 'selected' : '' }}>Dorm (monthly)</option>
                        <option value="1" {{ old('is_transient', 0) == 1 ? 'selected' : '' }}>Transient (daily)</option>
                    </select>
                </div>
