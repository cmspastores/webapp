...
<div class="container">
    ...
    <div id="search-refresh" class="search-refresh">
        <form method="GET" action="{{ route('rooms.index') }}" class="search-container">
            ...
            <div class="custom-dropdown">
                <div class="selected">{{ request('room_order') ? ucfirst(request('room_order')) : 'Ascending (1 → 50)' }}</div>
                <div class="dropdown-options">
...
.custom-dropdown .selected { padding:8px 12px; border:1px solid #E6A574; border-radius:8px; background:#fff; color:#5C3A21; transition:0.2s; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
/* 🌿 Room Table */
.room-table { width:100%; border-collapse:separate; border-spacing:0; text-align:center; background:transparent; border-radius:12px; overflow:hidden; }
.room-table thead { background:linear-gradient(to right,#F4C38C,#E6A574); color:#5C3A21; }
...
.room-table th:first-child, .room-table td:first-child { display:none; }
